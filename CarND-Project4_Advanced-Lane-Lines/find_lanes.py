...
def find_lane(binary_warped,left_fit,right_fit,mask):
    if (left_fit == '' or right_fit == ''):
        ...
        left_lanex = left_fit[0]*binary_warped.shape[0]**2 + left_fit[1]*binary_warped.shape[0] + left_fit[2]
        right_lanex = right_fit[0]*binary_warped.shape[0]**2 + right_fit[1]*binary_warped.shape[0] + right_fit[2]
        
        y_eval = np.max(ploty)
        ym_per_pix = 30/720 #meters per pixel in y dimension
        xm_per_pix = 3.7/(right_lanex-left_lanex) #meters per pixel in x dimension
        
        locx = (binary_warped.shape[1]/2 - ((left_lanex+right_lanex)/2))*xm_per_pix
        ...
        return window_img,left_fit,right_fit,mask,curverad, locx
        ...
        masked_image = cv2.bitwise_and(binary_warped, mask[:,:,1])
